        ...
        try:
            ...
            if columnas_faltantes:
                ...
                if st.button("💾 Cargar Referencia", type="primary"):
                    ...
                    cargar_nueva_referencia_psycopg2(df_ref, tipo_ref, fuente_ref, archivo_referencia.name)
...
#RRV01
# ========================================
# FUNCIÓN PARA AGREGAR AL FINAL DE TU ARCHIVO
# (Antes del if __name__ == "__main__":)
# ========================================

def cargar_nueva_referencia_psycopg2(df_ref, tipo_ref, fuente_ref, nombre_archivo):
    """
    Función de reemplazo usando psycopg2 directo
    AGREGAR ESTA FUNCIÓN AL FINAL DE TU ARCHIVO
    """
    
    st.markdown("## 🔄 REEMPLAZO CON PSYCOPG2")
    
    try:
        import psycopg2
        
        # Validar datos
        st.info("🔍 Validando datos...")
        
        columnas_requeridas = ['codigo_oficial', 'nombre_oficial']
        columnas_faltantes = [col for col in columnas_requeridas if col not in df_ref.columns]
        ...
        st.info("🔌 Conectando con psycopg2...")
        ...
        st.info(f"""
        **📊 Estado actual:**
        - Referencias existentes de {tipo_ref}: {total_existentes:,}
        - Nuevas a cargar: {len(df_ref):,}
        """)
        
        if total_existentes > 0:
            st.warning(f"""
            ⚠️ **SE ELIMINARÁN {total_existentes:,} REFERENCIAS EXISTENTES**
            Solo quedarán las {len(df_ref):,} nuevas referencias.
            """)
            
            confirmar = st.checkbox(
                f"✅ Confirmo ELIMINAR {total_existentes:,} y cargar {len(df_ref):,} nuevas",
                key=f"confirmar_psycopg2_{tipo_ref}_{int(time.time())}"
            )
            
            if not confirmar:
                st.warning("⏸️ Proceso cancelado")
                return
        
        # PROCESO PRINCIPAL
        st.info("🔄 Iniciando reemplazo...")
        
        progress = st.progress(0)
        status = st.empty()
        
        # 1. ELIMINAR referencias existentes
        status.text(f"🗑️ Eliminando {total_existentes:,} referencias existentes...")
        progress.progress(0.2)
        
        cursor.execute("DELETE FROM referencias_normalizacion WHERE tipo_catalogo = %s", (tipo_ref,))
        eliminados = cursor.rowcount
        
        st.info(f"✅ Eliminados: {eliminados:,} registros")
        
        # 2. INSERTAR nuevas referencias
        status.text(f"📥 Insertando {len(df_ref):,} nuevas referencias...")
        progress.progress(0.5)
        ...
        for idx, row in df_ref.iterrows():
            ...
            if registros_insertados % 100 == 0:
                progreso = 0.5 + (0.4 * registros_insertados / len(df_ref))
                progress.progress(progreso)
                status.text(f"📥 Insertados {registros_insertados:,} / {len(df_ref):,}...")
        ...
        status.text("🔍 Verificando resultado...")
        ...
        # RESULTADO
        if total_final == len(df_ref):
            st.success(f"""
            ## 🎉 REEMPLAZO EXITOSO CON PSYCOPG2
            
            **📊 Resultado:**
            - **Eliminadas:** {eliminados:,} referencias anteriores
            - **Insertadas:** {registros_insertados:,} nuevas referencias
            - **Total final:** {total_final:,} referencias de {tipo_ref}
            - **Verificación:** ✅ Correcto ({total_final} = {len(df_ref)})
            
            **✅ Solo existen las referencias más recientes de {tipo_ref}**
            """)
        else:
            st.error(f"❌ Error: Esperadas {len(df_ref)}, encontradas {total_final}")
...
# ========================================
# TAMBIÉN AGREGAR ESTA FUNCIÓN PARA ELIMINAR ARCHIVOS
# ========================================

def eliminar_archivo_procesado_psycopg2(id_archivo):
    """
    Eliminación de archivos usando psycopg2 directo
    TAMBIÉN AGREGAR ESTA FUNCIÓN
    """
    
    st.markdown("### 🗑️ Eliminación con psycopg2")
    
    if st.checkbox("⚠️ Confirmar eliminación definitiva", key=f"confirm_psycopg2_{id_archivo}"):
        
        if st.button("🗑️ ELIMINAR", key=f"psycopg2_delete_{id_archivo}", type="primary"):
            ...
            try:
                ...
                # Obtener info
                status.text("📋 Obteniendo información...")
                progress.progress(0.2)
                ...
                # Eliminar archivo
                status.text("🗑️ Eliminando archivo...")
                progress.progress(0.7)
                ...
                if verificacion == 0:
                    ...
                    **Archivo:** {nombre}
                    **Resultados eliminados:** {resultados_eliminados:,}
                    **Verificación:** ✅ Archivo no existe
                    """)
